...
inline static void InitializeExpectations(int expectation_count) {
  ...
  if (expectation_count > 0)
    expectations = (ExpectationStruct*)calloc(expectation_count, sizeof(ExpectationStruct));
  else
    expectations = NULL;
...
static void SetUp(int expectation_count) { InitializeExpectations(expectation_count); }
...
inline static void FailWhenNotAllExpectationsUsed(void) {
  ...
  snprintf(message, sizeof(message), format, set_expectation_count, get_expectation_count);
  Fail(message);
}
...
static void ExpectWrite(IoAddress offset, IoData data) {
  if (!FailWhenNoRoomForExpectations("mockIoData->ExpectWrite: Too many expectations"))
    RecordExpectation(kWrite, offset, data);
}

static void ExpectAdd(IoAddress offset, IoData data) {
  if (!FailWhenNoRoomForExpectations("mockIoData->ExpectAdd: Too many expectations"))
    RecordExpectation(kAdd, offset, data);
}

static void ExpectRemove(IoAddress offset, IoData data) {
  if (!FailWhenNoRoomForExpectations("mockIoData->ExpectRemove: Too many expectations"))
    RecordExpectation(kRemove, offset, data);
}

static void ExpectReadThenReturn(IoAddress offset, IoData to_return) {
  if (!FailWhenNoRoomForExpectations("mockIoData->ExpectReadThenReturn: Too many expectations"))
...
static void FailWhenNoUnusedExpectations(const char* format) {
  ...
  int offset = snprintf(message, size, "Count %d: No more expectations but was ", get_expectation_count + 1);
...
static bool ExpectedMatchesActual(void) {
  return (actual.kind == expected.kind) && (actual.offset == expected.offset) &&
         ((actual.data == expected.data) || (actual.data == kNoExpectedValue) || (expected.data == kNoExpectedValue));
}
...
static void FailWhenExpectedDoesNotMatch(void) {
  ...
  offset += snprintf(message + offset, size - offset, kExpectedMethodMismatchReports[expected.kind].report_expected,
                     expected.offset, expected.data);
  offset += snprintf(message + offset, size - offset, kExpectedMethodMismatchReports[actual.kind].report_but_was,
...
static void Write8bit(IoAddress offset, IoData data) { Write(offset, (uint8_t)data); }

static void Write16bit(IoAddress offset, IoData data) { Write(offset, (uint16_t)data); }

static void Write32bit(IoAddress offset, IoData data) { Write(offset, (uint32_t)data); }
...
static void Add8bit(IoAddress offset, IoData data) { Add(offset, (uint8_t)data); }

static void Add16bit(IoAddress offset, IoData data) { Add(offset, (uint16_t)data); }

static void Add32bit(IoAddress offset, IoData data) { Add(offset, (uint32_t)data); }
...
static void Remove8bit(IoAddress offset, IoData data) { Remove(offset, (uint8_t)data); }

static void Remove16bit(IoAddress offset, IoData data) { Remove(offset, (uint16_t)data); }

static void Remove32bit(IoAddress offset, IoData data) { Remove(offset, (uint32_t)data); }
