...
const ComparerInterface kComparer = {Compare};
...
const DeleterInterface kDeleter = {Delete};
...
TEST_F(ListTest, FindIfComparatorIsNotSet) {
  ComparerInterface ci = {NULL};
  list.SetItemComparer(l, &ci);
  AddAllItems();
...
TEST_F(ListTest, DeleteIfDeleterIsNotSet) {
  DeleterInterface di = {NULL};
  list.SetItemDeleter(l, &di);
  AddAllItems();
